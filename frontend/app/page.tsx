...
import { getCookie } from '@/utils/cookie' // Assegura't que aquesta utilitat existeixi o usa una llibreria
...
export default function Home() {
  // Si hi ha sessió, redirigir al dashboard (client)
  useEffect(() => {
    // Nota: Assegura't que getCookie estigui ben importat
      ...
      <main className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-12 p-6 md:flex-row md:items-center md:justify-between md:p-12">
        <section className="flex-1 flex flex-col justify-center gap-6 md:max-w-lg">
          ...
          <div className="flex gap-4">
            <Link 
              href="/login" 
              className="inline-flex items-center justify-center rounded-xl bg-amber-600 px-6 py-3 text-white font-semibold shadow hover:bg-amber-700"
            >
              Login
            </Link>
            <Link 
              href="/register" 
              className="inline-flex items-center justify-center rounded-xl bg-zinc-900 px-6 py-3 text-white font-semibold shadow hover:bg-zinc-700"
            >
              Register
            </Link>
          </div>
